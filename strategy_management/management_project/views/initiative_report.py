...
# -------------------- LIST INITIATIVE REPORTS --------------------
@login_required
@role_required(['viewer', 'editor', 'owner', 'admin'], model_name='initiative_report', action='view')
...
@login_required
@role_required(['editor', 'owner', 'admin'], model_name='initiative_report', action='create')
...
# -------------------- UPDATE INITIATIVE REPORT --------------------
@login_required
@role_required(['editor', 'owner', 'admin'], model_name='initiative_report', action='edit')
...
# -------------------- DELETE INITIATIVE REPORT --------------------
@login_required
@role_required(['owner', 'admin'], model_name='initiative_report', action='delete')
def delete_initiative_report(request, pk):
    ...
    return render(request, 'initiative_report/delete_confirm.html', {
        'timeline': timeline,
        'permissions': permissions,
    })

# -------------------- EXPORT INITIATIVE REPORT TO EXCEL --------------------
@login_required
@role_required(['viewer', 'editor', 'owner', 'admin'], model_name='initiative_report', action='view')
def export_initiative_report_to_excel(request):
    ...
    # ===== Title Row =====
    total_columns = 24  # update to match all columns
    ...
    headers = [
        ...
        "Planned HR", "Actual HR", "Remaining HR", "HR Utilization (%)",
    ...
    for r in reports:
        row = [
            ...
            float(r.hr_utilization_percent),
...
# -------------------- INITIATIVE REPORT CHARTS & ANALYTICS --------------------
@login_required
@role_required(['viewer', 'editor', 'owner', 'admin'], model_name='initiative_report', action='view')
